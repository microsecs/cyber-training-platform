...
export default function SendTrainingReminder({assignmentId,disabled=false}:{assignmentId:string;disabled?:boolean}){
 ...
 return <div><button onClick={send} disabled={disabled||busy} className="rounded-lg border border-cyan-400/30 bg-cyan-400/10 px-3 py-2 text-xs font-semibold text-cyan-300 disabled:opacity-40">{busy?"Sending...":"Send Reminder"}</button>{msg&&<div className="mt-1 text-xs text-slate-500">{msg}</div>}</div>;
}